...
Page({
	...
	handleTimeUpdate(event) {
		...
		* */
		...
		if (videoItem && event.detail.currentTime !== 0) { // 之前有
			videoItem.currentTime = event.detail.currentTime;
		} else if(!videoItem && event.detail.currentTime === 0) { // 之前没有
			videoUpdateTime.push(videoTimeObj);
		}else{
			console.log(videoItem,event.detail.currentTime);  // 这个判断就TM离谱（半天就这么度过了~）
		}
