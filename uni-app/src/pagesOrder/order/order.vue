<script setup lang="ts">
    ...
    // 提交订单
    const onPostMemberOrder = async () => {
        // 发送请求
        const res = await postMemberOrderApi({
            addressId: selectedAddress.value!.id,
            buyerMessage: buyerMessage.value,
            deliveryTimeType: activeDelivery.value.type,
            goods: orderPreList.value!.goods.map((v) => ({ count: v.count, skuId: v.skuId })),
            payChannel: 2,
            payType: 1,
        })
...
<template>
    <scroll-view scroll-y class="viewport">
        ...
        <view class="settlement">
            <view class="item">
                ...
                <text class="number symbol">{{ orderPreList?.summary.totalPrice }}</text>
            </view>
            <view class="item">
                <text class="text">运费: </text>
                <text class="number symbol">{{ orderPreList?.summary.postFee }}</text>
            </view>
        </view>
    </scroll-view>
    ...
    <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <view class="total-pay symbol">
            <text class="number">{{ orderPreList?.summary.totalPayPrice }}</text>
        </view>
        <view class="button" :class="{ disabled: !selectedAddress }" @tap="onPostMemberOrder">
            提交订单
        </view>
    </view>
</template>
